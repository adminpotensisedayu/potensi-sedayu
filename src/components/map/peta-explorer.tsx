...
import { Layers, LocateFixed, X, Store, School, Map } from "lucide-react"
...
function buildPopup(p: MapPoint): string {
  ...
  const sectorLabel = p.sector === "umkm" ? "UMKM" : "Sekolah"
  const alamat = p.alamat ? p.alamat.slice(0, 85) + (p.alamat.length > 85 ? "…" : "") : ""

  const fotoBlock = p.foto
    ? `<div style="height:112px;border-radius:8px;overflow:hidden;margin-bottom:10px;background:#e5e7eb;flex-shrink:0;">
    ...
    <div style="width:205px;font-family:system-ui,-apple-system,sans-serif;line-height:1.4;">
      ...
      <div style="display:flex;align-items:center;gap:6px;margin-bottom:5px;">
        <span style="
          ...
          background:${clr}18;color:${clr};border:1px solid ${clr}33;flex-shrink:0;
        ">${sectorLabel}</span>
        ${p.subLabel ? `<span style="font-size:11px;color:#888;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${p.subLabel}</span>` : ""}
      </div>
      <p style="font-weight:700;font-size:13.5px;margin:0 0 3px;color:#111;overflow:hidden;text-overflow:ellipsis;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;">${p.label}</p>
      ...
      <a href="${p.href}"
        style="
          ...
          background:${clr};color:white;border-radius:8px;
  ...
  return (
    <>
      {shown.map((p) => (
        ...
        >
          <Popup minWidth={205} maxWidth={220} className="custom-popup-reset">
      ...
      <div className="absolute left-1/2 top-4 z-[1000] -translate-x-1/2">
        <div className="flex items-center gap-1 rounded-2xl border border-border bg-background/96 p-1 shadow-xl backdrop-blur-md">
          {(
            ...
            >
              {label}
              <span className={`text-[10px] ${filter === id ? "opacity-75" : "opacity-60"}`}>
                ({n})
              </span>
            </button>
          ))}
        ...
        {userPos && (
          <>
            <Marker position={userPos} icon={PIN_USER}>
              <Popup>
                <div style={{ fontFamily: "system-ui", fontSize: "13px", fontWeight: 700, color: "#3b82f6", padding: "2px 0" }}>
                  📍 Lokasi Anda
                </div>
              </Popup>
            </Marker>
        ...
        <div className="absolute bottom-12 left-4 z-[1000]">
          <div className="flex items-center gap-3 rounded-2xl border border-border bg-background/96 px-4 py-2.5 shadow-xl backdrop-blur-md">
            <div className="flex items-center gap-1.5">
              <span className="flex size-3 items-center justify-center rounded-full bg-amber-400 ring-1 ring-white" />
              <span className="text-[11px] font-medium text-muted-foreground">UMKM</span>
            </div>
            <div className="h-3 w-px bg-border" />
            <div className="flex items-center gap-1.5">
              <span className="flex size-3 items-center justify-center rounded-full bg-teal-400 ring-1 ring-white" />
            ...
            {userPos && (
              <>
                <div className="h-3 w-px bg-border" />
                <div className="flex items-center gap-1.5">
                  <span className="flex size-3 items-center justify-center rounded-full bg-blue-500 ring-1 ring-white" />
