...
import {
  ...
  Camera, CheckCircle2, AlertCircle, Navigation, Clock,
...
// ─── ImageUpload mini (inline, tanpa dependency admin) ────────
function PhotoUpload({
  value, onChange, label, index,
}: {
  value: string | null
        ...
        <div className="relative aspect-[4/3] overflow-hidden rounded-xl border border-border bg-muted">
          ...
          <div className="absolute inset-0 flex items-center justify-center gap-2 bg-black/0 opacity-0 transition group-hover:bg-black/40 group-hover:opacity-100">
            <button type="button" onClick={() => inputRef.current?.click()}
              className="flex size-9 items-center justify-center rounded-full bg-white/90 text-foreground transition hover:bg-white">
              <Upload className="size-4" />
            </button>
            <button type="button" onClick={() => onChange(null)}
              className="flex size-9 items-center justify-center rounded-full bg-white/90 text-destructive transition hover:bg-white">
              <X className="size-4" />
            </button>
          </div>
        </div>
        ...
        <button type="button" onClick={() => inputRef.current?.click()}
          ...
          {loading ? (
            ...
            <>
              ...
              <span className="text-[10px] opacity-60">Maks 5MB</span>
            </>
          )}
        </button>
      )}
...
export function PengajuanForm({ kategoriList, subKategoriList, waNumber = "" }: Props) {
  // Konversi 08xxx → 628xxx untuk link WA
  const waLink = (() => {
  ...
  // Saat kategori ganti, reset sub kategori
  ...
  const handleGeolocate = useCallback(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => { setGeoLoading(false); alert("Tidak bisa akses lokasi. Izinkan lokasi di browser.") },
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!foto1)                 { setError("Minimal 1 foto usaha wajib diunggah."); return }
  ...
  if (success) {
    return (
      <div className="flex flex-col items-center py-16 text-center">
        ...
        <h2 className="mb-2 font-serif text-2xl text-foreground">Pengajuan Berhasil Dikirim!</h2>
        ...
        <p className="mb-6 max-w-sm text-sm text-muted-foreground">
          ...
          Kami akan menghubungi Anda via WhatsApp <strong className="text-foreground">{whatsapp}</strong> dalam 1–3 hari kerja.
        </p>

        <div className="mb-8 w-full max-w-sm rounded-2xl border border-border bg-card p-5">
          <p className="mb-3 text-xs font-semibold uppercase tracking-widest text-muted-foreground">Yang perlu Anda ketahui</p>
          {[
            "Tim kami akan memverifikasi data usaha Anda",
            "Anda akan dihubungi via WhatsApp jika ada pertanyaan",
            "Usaha tayang otomatis setelah disetujui",
          ].map((t, i) => (
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      {/* ── Bagian 1: Pemilik ── */}
      <Section icon={User} title="Informasi Pemilik"
        desc="Data diri Anda sebagai pemilik usaha"
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <Field label="Nomor WhatsApp" required hint="Contoh: 08123456789 atau +628123456789">
            <input type="tel" value={whatsapp} onChange={(e) => setWhatsapp(e.target.value)}
              placeholder="08xx-xxxx-xxxx" className={inputCls} />
          </Field>
          <div className="sm:col-span-2">
            <Field label="Email" hint="Opsional — untuk notifikasi tambahan">
      ...
      {/* ── Bagian 2: Usaha ── */}
      <Section icon={Store} title="Informasi Usaha"
        desc="Detail usaha yang ingin didaftarkan"
        ...
        <div className="space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Field label="Kategori" required>
              <div className="relative">
                <select value={kategoriId} onChange={(e) => handleKategoriChange(e.target.value)}
                  className={selectCls}>
          ...
          <Field label="Deskripsi Usaha" hint="Ceritakan produk/layanan yang Anda tawarkan">
            <textarea rows={4} value={deskripsi} onChange={(e) => setDeskripsi(e.target.value)}
              placeholder="Jelaskan usaha Anda — produk, keunikan, atau keunggulan..."
      ...
      <Section icon={MapPin} title="Lokasi Usaha"
        desc="Alamat dan koordinat untuk ditampilkan di peta"
          ...
          <div>
            <div className="mb-1.5 flex items-center justify-between">
              <label className="text-sm font-medium text-foreground">
                Koordinat <span className="ml-1 text-xs font-normal text-muted-foreground">(opsional — untuk pin di peta)</span>
              </label>
              <button type="button" onClick={handleGeolocate} disabled={geoLoading}
                className="flex items-center gap-1.5 rounded-lg border border-border bg-background px-3 py-1.5 text-xs font-semibold text-foreground transition hover:bg-muted disabled:opacity-60">
                {geoLoading
            ...
            <div className="grid grid-cols-2 gap-3">
              <input type="number" step="any" value={latitude}
                ...
                placeholder="Latitude (-7.xxxxx)"
                className={inputCls} />
              <input type="number" step="any" value={longitude}
                onChange={(e) => setLongitude(e.target.value)}
                placeholder="Longitude (110.xxxxx)"
                className={inputCls} />
            </div>
            <p className="mt-1.5 flex items-start gap-1.5 text-[11px] text-muted-foreground">
              <Info className="mt-0.5 size-3 shrink-0" />
              Atau buka Google Maps, cari lokasi usaha Anda, klik kanan → salin koordinat
            </p>
          </div>
        </div>
      </Section>
      ...
      <Section icon={Camera} title="Foto Usaha"
        desc="Minimal 1 foto wajib · Maks 3 foto · Format JPG/PNG/WEBP"
        ...
        <p className="mt-3 text-xs text-muted-foreground">
          💡 Tips: gunakan foto tampak depan usaha, produk unggulan, atau suasana toko untuk hasil terbaik.
        </p>
      </Section>

      {/* ── Bagian 5: Kelengkapan ── */}
        ...
        <div className="space-y-4">
          ...
          <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-border bg-background/50 p-4 transition hover:bg-muted/40">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Centang jika usaha Anda sudah memiliki dokumen perizinan resmi
              </p>
            </div>
          </label>
          ...
          <Field label="Catatan Tambahan" hint="Pesan untuk tim peninjau (opsional)">
            <textarea rows={3} value={catatan} onChange={(e) => setCatatan(e.target.value)}
              placeholder="Ada hal khusus yang ingin Anda sampaikan kepada tim kami?"
              className={inputCls + " resize-none"} />
          </Field>
        </div>
      </Section>
      ...
      <div className="space-y-4 rounded-2xl border border-border bg-card p-6">
        <div className="flex items-start gap-3 rounded-xl bg-muted/40 p-3">
          ...
          <p className="text-xs leading-relaxed text-muted-foreground">
            Dengan mengirim formulir ini, Anda menyatakan bahwa informasi yang diberikan adalah benar dan akurat.
            Data Anda hanya digunakan untuk keperluan direktori Desa Sedayu dan tidak akan dibagikan ke pihak ketiga.
          </p>
        </div>

        <button type="submit" disabled={submitting}
          className="group relative flex w-full items-center justify-center gap-3 overflow-hidden rounded-xl bg-gradient-to-r from-teal-600 to-emerald-500 px-6 py-4 text-sm font-bold text-white shadow-[0_8px_24px_-8px_rgba(13,148,136,0.5)] transition hover:-translate-y-[1px] hover:shadow-[0_12px_30px_-8px_rgba(13,148,136,0.6)] active:scale-[.99] disabled:cursor-wait disabled:opacity-80">
        ...
        <p className="text-center text-xs text-muted-foreground">
          Butuh bantuan?{" "}
          <a href={waLink ? "https://wa.me/" + waLink : "#"} target="_blank" rel="noopener noreferrer"
