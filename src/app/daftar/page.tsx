...
export const metadata = {
  title: "Daftarkan Usaha Anda — Potensi Sedayu",
  description: "Daftarkan UMKM Anda ke direktori resmi Desa Sedayu. Gratis, mudah, dan langsung tayang setelah diverifikasi.",
}

export default async function DaftarPage() {
  const supabase = await createClient()

  const [{ data: kategoriList }, { data: subKategoriList }, { data: kontak }] = await Promise.all([
        ...
        <div className="relative mx-auto max-w-4xl px-6 py-12 text-center">
          ...
          <div className="mt-6 flex flex-wrap items-center justify-center gap-3">
            {[
              ...
              { icon: Clock,       text: "Review 1–3 hari kerja" },
              { icon: Phone,       text: "Notif via WhatsApp" },
            ].map(({ icon: Icon, text }) => (
              <div key={text} className="flex items-center gap-1.5 rounded-full bg-background/80 border border-border px-3 py-1.5 text-xs font-medium text-muted-foreground shadow-sm backdrop-blur-sm">
